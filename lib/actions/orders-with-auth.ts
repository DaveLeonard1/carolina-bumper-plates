;```ts file="lib/actions/orders-fallback.ts"
[v0-no-op-code-block-prefix]'use server';
...
export async function updateOrder(id: string, prevState: State, formData: FormData) {
  ...
  try {
    await sql`
      UPDATE orders
      SET customer_id = ${customer_id}, customer_name = ${customer_name}, amount = ${amountInCents}, status = ${status}
      WHERE id = ${id}
    `
  } catch (error) {
...
export async function submitOrder(prevState: State, formData: FormData) {
  ...
  try {
    ...
    await sql`
      ...
      VALUES (${order.customer_id}, ${order.customer_name}, ${order.first_name}, ${order.last_name}, ${order.amount}, ${order.status}, ${order.date})
    `
  } catch (error) {
    return {
      message: "Database Error: Failed to Create Order.",
