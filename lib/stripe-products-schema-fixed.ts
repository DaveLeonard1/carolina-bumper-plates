...
export interface StripeProductData {
  ...
  name: string
  description?: string
  selling_price: number // Changed from price_per_lb to selling_price
  weight: number
...
export const createOrUpdateStripeProduct = async (productData: StripeProductData) => {
  try {
    ...
    // Validate required fields using correct schema
    if (!productData.id) {
    ...
    console.log(
      `Processing product: ID=${productData.id}, Weight=${productData.weight}lb, Price=$${productData.selling_price}`,
    )
    ...
    // Create price for the product using selling_price
...
export const syncAllProductsWithStripe = async () => {
  try {
    ...
    let products
    if (hasStripeColumns) {
      // Query with Stripe columns if they exist - using correct column names
      const { data, error } = await supabaseAdmin
        .from("products")
        .select("*")
        .not("weight", "is", null)
        .not("selling_price", "is", null) // Changed from price_per_lb to selling_price
      ...
      // Query without Stripe columns if they don't exist - using correct column names
      const { data, error } = await supabaseAdmin
        .from("products")
        .select("*")
        .not("weight", "is", null)
    ...
    products.forEach((product, index) => {
      console.log(`Product ${index + 1}:`, {
        ...
        selling_price: product.selling_price, // Changed from price_per_lb
    ...
    for (const product of products) {
      try {
        // Double-check validation using correct schema
        if (!product.id) {
        ...
        if (!product.selling_price || typeof product.selling_price !== "number") {
          console.error(`❌ Skipping product ${product.id} with invalid selling_price:`, product.selling_price)
          continue
        }
...
export const getStripeProductForWeight = async (weight: number) => {
  try {
    ...
    // Validate product data using correct schema
    if (!product.id || !product.weight || !product.selling_price) {
      console.error("Invalid product data - missing required fields:", {
        id: product.id,
        weight: product.weight,
